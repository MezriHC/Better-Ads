      ...
      >
        ...
        >
          <div className={`${config.padding} bg-gradient-to-b from-transparent to-white/10 dark:to-black/10 rounded-[12px] flex items-center justify-center ${config.gap}`}>
            {icon && (
              <span className={`${config.iconSize} shrink-0 flex items-center justify-center ${disabled ? 'text-muted-foreground' : 'text-background'}`}>
                {icon}
              </span>
            )}
            <span className={`font-semibold ${config.textSize} ${disabled ? 'text-muted-foreground' : 'text-background'}`}>
              {children}
            </span>
          </div>
        </div>
