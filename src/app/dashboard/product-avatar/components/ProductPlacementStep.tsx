...
export function ProductPlacementStep({ selectedAvatar, onBack, onNext }: ProductPlacementStepProps) {
  ...
  const handleImageUpload = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && file.type.startsWith('image/')) {
      ...
      setGeneratedPhotos([]) // Reset generated photos on new upload
        ...
        <div className="bg-card border border-border rounded-xl p-4 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="relative w-full h-64 rounded-lg overflow-hidden bg-muted">
              {!productImageUrl ? (
                <div 
                  className={`border-2 border-dashed rounded-lg h-full transition-all cursor-pointer ${
                    isDragOver 
                ...
                >
                  ...
                  >
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-colors ${
                      ...
                      <IconUpload className={`w-6 h-6 transition-colors ${
            ...
            <div className="bg-accent/20 rounded-lg p-3 flex flex-col gap-2">
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm font-medium text-foreground">
                  {productImageUrl ? 'Product photo ready for AI generation' : 'Product photo not uploaded'}
                </p>
              </div>
              <p className="text-xs text-muted-foreground">
                {productImageUrl 
                  ? `The AI will place your product in the avatar's hands across 4 different poses and angles.`
                  : 'Upload a clear product photo to start AI generation with your selected avatar.'
                }
              </p>
            </div>
